...
var f = class extends HTMLElement {
	...
	_renderModeTabs() {
      ...
      `, i = e.querySelector(".tab-bubble"), e.querySelectorAll("[data-mode]").forEach((e) => e.addEventListener("click", (t) => {
		...
		}), i && s && (i.className = `tab-bubble bubble-${this._mode}`, requestAnimationFrame(() => {
			let e = s.offsetLeft, t = s.offsetWidth;
			t > 0 && (i.style.width = `${t}px`, i.style.transform = `translate3d(${e}px, 0, 0)`);
		}));
	}
	_currentModeConfig() {
		let e = {
