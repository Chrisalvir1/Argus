...
var f = class extends HTMLElement {
	...
	async _load() {
		let e = this.shadowRoot.querySelector(".wrap");
		e && (e.style.setProperty("display", "none", "important"), e.classList.remove("wrap-ready"));
		let t;
		try {
			t = await this._send("argus/login_bootstrap");
		} catch (e) {
		...
		this._currentUserTheme = n, n.background_mode !== "default" && n.background_file ? (this._backgroundMode = n.background_mode, this._panelBgFile = n.background_file, this._backgroundImages = [n.background_file]) : n.background_mode !== "default" && n.background_mode ? (this._backgroundMode = n.background_mode, this._panelBgFile = "", this._backgroundImages = []) : (this._backgroundMode = t.background_mode || "none", this._panelBgFile = "", this._backgroundImages = t.background_images || []), t.weather_source && (this._weatherSource = t.weather_source), t.temperature_source && (this._temperatureSource = t.temperature_source);
		try {
			let e = localStorage.getItem("argus_lang");
			this._manualLang = e && e !== "auto" ? e : t.language && t.language !== "auto" && l.some((e) => e.code === t.language) ? t.language : null;
		} catch {}
		if (this._updateCanvasBackground(), t.configuration_missing) {
		...
		if (t.legacy_claim_needed) {
			this._loadState = "legacy_claim", this._renderLegacyClaimScreen();
			return;
		}
		let r = this.hasAttribute("compact") || this.classList.contains("argus-compact") || !!this._cardConfig?.compact;
		if (!t.has_active_session || !this._profileSelectedThisMount) {
			if (r) {
				this._profileSelectedThisMount = !0;
				let e = (t.users || []).find((e) => e.is_own_profile) || (t.users || [])[0];
				if (e && !t.has_active_session) try {
					await this._send("argus/select_profile", { argus_user_id: e.id });
				} catch {}
			} else {
				this._loadState = "profile_selection", this._renderLoginScreen(t);
				return;
			}
		}
		...
		if (r && i?.entries?.length) {
			let e = this._config?.entity || "alarm_control_panel.argus", t = i.entries.find((t) => t.entity_id === e);
			t ||= i.entries[0], i.entries = [t];
		}
		this._dashboard = i, this._available = i.available_entities || [], this._ui = i.ui || {
			modes: {},
			dashboard: {}
		}, this._notifTargets = i.ui?.notif_targets || [], this._users = Array.isArray(i.ui?.users) ? i.ui.users.filter((e) => e && typeof e == "object" && !Array.isArray(e)) : [], this._homeName = i.ui?.home_name || "", this._emergencyNumber = i.ui?.emergency_number || "911", this._loadState = "dashboard";
		let a = this.shadowRoot.querySelector(".wrap");
		a && (a.style.removeProperty("display"), a.classList.add("wrap-ready")), this._hideInitialCurtain(), this._currentProfile = i.current_profile || null;
		let o = this.shadowRoot.getElementById("bootstrap-overlay");
		o && (this._currentProfile && !this._welcomeShownThisMount ? o.style.display = "flex" : o.style.display = "none"), await this._loadActivityTimeline(i.entry_id), this._panicOutputs = i.ui?.panic_outputs || [];
		let s = this._users.find((e) => e.id === this._currentProfile?.id) || {};
		this._backgroundMode = s.background_mode || i.ui?.background_mode || "weather", this._backgroundImages = s.background_images || i.ui?.background_images || [], this._temperatureSource = i.ui?.temperature_source || "auto", this._weatherSource = i.ui?.weather_source || "auto", this._clockFormat = i.ui?.clock_format || "auto", this._panelBgFile = s.panel_bg_file === void 0 ? i.ui?.panel_bg_file || "" : s.panel_bg_file, this._panelBgSound = !!(s.panel_bg_sound === void 0 ? i.ui?.panel_bg_sound : s.panel_bg_sound);
		let c = s.hub_bg_mode || i.ui?.hub_bg_mode || "none";
		this._hubBgMode = c === "none" || c === "default" ? "default" : c, this._hubBgFile = s.hub_bg_file === void 0 ? i.ui?.hub_bg_file || "" : s.hub_bg_file, this._hubBgSound = !!(s.hub_bg_sound === void 0 ? i.ui?.hub_bg_sound : s.hub_bg_sound), this._updateTheme(), this._updateHomeNameDisplay(), this._updateProfileBadge(), this._populateTemperatureSources();
		let u = this.shadowRoot.getElementById("temp-source-select-standalone");
		u && (u.value = this._temperatureSource || "auto", u.dataset.bound || (u.dataset.bound = "1", u.addEventListener("change", () => this._savePersonalization())));
		let d = this.shadowRoot.getElementById("argus-clock-format-select");
		d && (this._clockFormat = this._ui?.clock_format || this._dashboard?.clock_format || "auto", d.value = this._clockFormat, d.dataset.bound || (d.dataset.bound = "1", d.addEventListener("change", () => this._savePersonalization())));
		let f = this.shadowRoot.getElementById("emergency-number-input");
		f && (f.value = this._emergencyNumber), this._renderSosOutputs(), this._configureEmergencyCall();
		let p = this.shadowRoot.getElementById("bg-mode-select-standalone");
		p && (p.innerHTML = `
        <option value="none">${this._t("bg_panel_none")}</option>
      `, p.value = this._backgroundMode || "none");
		let m = this.shadowRoot.getElementById("hub-bg-mode-select");
		m && (m.innerHTML = `
        <option value="default">${this._t("bg_hub_default")}</option>
        <option value="image">${this._t("bg_image_opt")}</option>
      `, m.value = this._hubBgMode || "default");
		let h = this.shadowRoot.getElementById("panel-bg-url-input");
		h && (h.value = this._panelBgFile && !this._panelBgFile.startsWith("data:") ? this._panelBgFile : "");
		let g = this.shadowRoot.getElementById("chk-panel-bg-sound");
		g && (g.checked = this._panelBgSound);
		let _ = this.shadowRoot.getElementById("hub-bg-url-input");
		_ && (_.value = this._hubBgFile && !this._hubBgFile.startsWith("data:") ? this._hubBgFile : "");
		let v = this.shadowRoot.getElementById("chk-hub-bg-sound");
		v && (v.checked = this._hubBgSound), this._updateBgFieldsVisibility(), this._updateCanvasBackground(), this._isAdmin = i.current_profile?.role === "admin", this._permissions = i.current_profile?.permissions || {};
		let y = i.entries?.[0]?.entity_id;
		if (y ? ((!this._modeEntryId || this._modeEntryId === "default") && (this._modeEntryId = y), this._loadRetryTimeout &&= (clearTimeout(this._loadRetryTimeout), null)) : (!this._modeEntryId || this._modeEntryId === "default") && (this._modeEntryId = null), this._isAdmin) try {
			let e = await this._send("argus/get_ha_users");
			this._haUsersList = e?.ha_users || [];
		} catch (e) {
			console.warn("Could not fetch HA users:", e);
		}
		let b = i.entries?.[0]?.pin_configured === !0, x = this.shadowRoot.getElementById("current-pin-display"), S = this.shadowRoot.getElementById("group-current-pin");
		x && (x.textContent = b ? this._t("pin_active_yes") : this._t("pin_active_no")), S && (b ? S.classList.remove("collapsed") : S.classList.add("collapsed"));
		let C = this.shadowRoot.getElementById("lnk-forgot-pin");
		C && (C.style.display = b ? "inline" : "none");
		let w = this.shadowRoot.getElementById("pin-forgot-link");
		w && (w.style.display = b ? "inline" : "none"), [
	...
	async _runProfileWelcomeAnimation(e) {
		...
		t && (t.style.opacity = "0", t.style.transition = "opacity 0.25s ease, transform 0.35s cubic-bezier(0.34, 1.56, 0.64, 1)");
		let n = this.shadowRoot.querySelector(".wrap");
		n && (n.style.removeProperty("display"), n.classList.add("wrap-ready"));
		let r = document.createElement("div");
		r.className = "argus-welcome-screen active-anim", r.style.position = "fixed", r.style.inset = "0", r.style.background = "rgba(0,0,0,0.88)", r.style.display = "flex", r.style.flexDirection = "column", r.style.alignItems = "center", r.style.justifyContent = "center", r.style.zIndex = "10000", r.style.pointerEvents = "none", r.innerHTML = `
    ...
    `, this.shadowRoot.appendChild(r), await new Promise((e) => requestAnimationFrame(e)), this.shadowRoot.querySelectorAll(".argus-profile-overlay, .argus-pin-prompt").forEach((e) => e.remove());
		let i = Promise.resolve();
		this._dashboardLoading || (this._dashboardLoading = !0, i = this._load().catch((e) => console.error("Load error during animation:", e)).finally(() => {
			this._dashboardLoading = !1;
		}));
		let a = r.querySelector("#welcome-avatar-flying"), o = r.querySelector("#welcome-text-anim");
		if (a && o) {
			a.style.transform = "scale(0.8)", a.style.opacity = "0", o.style.opacity = "0", o.style.transform = "translateY(15px)", await new Promise((e) => requestAnimationFrame(e)), a.style.transition = "transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.4s ease", a.style.transform = "scale(1)", a.style.opacity = "1", o.style.transition = "transform 0.5s cubic-bezier(0.16, 1, 0.3, 1) 0.15s, opacity 0.5s ease 0.15s", o.style.transform = "translateY(0)", o.style.opacity = "1", await new Promise((e) => setTimeout(e, 1300)), await i, o.style.transition = "opacity 0.25s ease", o.style.opacity = "0";
			let e = a.getBoundingClientRect(), t = this.shadowRoot.getElementById("hero-profile-avatar") || this.shadowRoot.querySelector("#hero-profile-container .user-avatar") || this.shadowRoot.querySelector("#hero-profile-container img") || this.shadowRoot.querySelector("#hero-profile-container .hero-profile-pill"), n = window.innerWidth / 2, s = 60, c = .35;
			if (t) {
				let r = t.getBoundingClientRect();
				r.width > 0 && r.height > 0 && (n = r.left + r.width / 2, s = r.top + r.height / 2, c = Math.max(.2, r.width / e.width));
			}
			let l = n - (e.left + e.width / 2), u = s - (e.top + e.height / 2);
			a.style.transition = "transform 0.65s cubic-bezier(0.25, 1.25, 0.5, 1), opacity 0.35s ease 0.45s", a.style.transform = `translate(${l}px, ${u}px) scale(${c})`, r.style.transition = "background-color 0.55s ease 0.1s", r.style.backgroundColor = "transparent", await new Promise((e) => setTimeout(e, 650)), t && (t.style.opacity = "1", t.style.transform = "scale(1.1)", setTimeout(() => {
				t && (t.style.transform = "scale(1)");
			}, 180));
		}
		await new Promise((e) => setTimeout(e, 120)), await i, this._nukeAllLoginOverlays();
	}
