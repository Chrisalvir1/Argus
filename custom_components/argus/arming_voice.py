...
def _sensor_identity(hass,entity_id):
 state=hass.states.get(entity_id);entry=er.async_get(hass).async_get(entity_id);device=dr.async_get(hass).async_get(entry.device_id) if entry and entry.device_id else None;name=(device.name_by_user or device.name) if device else None
...
async def async_announce_alarm_triggered(hass,config_entry,*,alarm_entity_id,sensor_entity_id,triggered_sensor_ids,mode,source,additional=False):
 options=_options(hass,config_entry);sensor=_sensor_identity(hass,sensor_entity_id);all_info=[_sensor_identity(hass,e) for e in dict.fromkeys(triggered_sensor_ids)];mode_name=_MODE_NAMES.get(str(mode or ""),str(mode or "desconocido"));values={"sensor":sensor["name"],"sensors":", ".join(i["name"] for i in all_info),"count":len(all_info),"sensor_entity_id":sensor_entity_id,"sensor_state":sensor["state"],"mode":mode_name,"home":config_entry.title,"source":source,"triggered_by":source};key=CONF_ARMING_VOICE_MESSAGE_ADDITIONAL_TRIGGERED if additional else CONF_ARMING_VOICE_MESSAGE_TRIGGERED;default=DEFAULT_ARMING_VOICE_MESSAGE_ADDITIONAL_TRIGGERED if additional else DEFAULT_ARMING_VOICE_MESSAGE_TRIGGERED;message=_render(options.get(key,default),values);hass.bus.async_fire("argus_alarm_triggered",{"entry_id":config_entry.entry_id,"alarm_entity_id":alarm_entity_id,"mode":mode,"mode_name":mode_name,"sensor":sensor,"sensor_name":sensor["name"],"sensor_entity_id":sensor_entity_id,"triggered_sensors":all_info,"triggered_sensor_names":[i["name"] for i in all_info],"triggered_sensor_count":len(all_info),"additional":additional,"source":source,"message":message});await _async_speak(hass,options,message)
