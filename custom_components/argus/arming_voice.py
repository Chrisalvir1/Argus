...
def _options(hass,entry):
    merged=dict(entry.options); merged.update(hass.data.get(DOMAIN,{}).get("arming_voice_yaml",{})); return merged
def _sensor_identity(hass,entity_id):
    ...
    return {"entity_id":entity_id,"name":str(name or entity_id),"state":state.state if state else "unavailable","unique_id":entry.unique_id if entry else None,"device_id":entry.device_id if entry else None,"identifiers":sorted(f"{a}:{b}" for a,b in (device.identifiers if device else set())),"connections":sorted(f"{a}:{b}" for a,b in (device.connections if device else set()))}
...
async def async_announce_arming_wait_update(hass,config_entry,*,alarm_entity_id,target,previous_open,current_open):
    ...
    current_info=[_sensor_identity(hass,e) for e in current];prev_set,cur_set=set(previous),set(current);closed=[_sensor_identity(hass,e) for e in previous if e not in cur_set];opened=[_sensor_identity(hass,e) for e in current if e not in prev_set];names=[i["name"] for i in current_info];mode=_MODE_NAMES.get(target,target)
    values={"count":len(current),"total":len(prev_set|cur_set),"sensors":", ".join(names),"closed":", ".join(i["name"] for i in closed),"opened":", ".join(i["name"] for i in opened),"mode":mode,"home":config_entry.title};options=_options(hass,config_entry)
...
def install_cancel_announcement():
    """Compatibility no-op; cancellation now lives in safety_runtime."""
    return None
