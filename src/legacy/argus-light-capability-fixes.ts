...
async function runPhysicalFlashTest(panel,button,entityId,flashMode,rgbColor){
  if(!entityId||flashMode==='none')return; const original=button.textContent; button.disabled=true; button.textContent='Probando de forma segura…';
...
function capabilityTitle(capability,name){if(!capability.safeFlash)return`💡 Luz fija segura — ${escapeHtml(name)}`;return`${capability.color?'🎨 Color y destello seguro':'✨ Destello seguro'} — ${escapeHtml(name)}`;}
