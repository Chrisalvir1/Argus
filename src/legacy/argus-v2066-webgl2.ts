...
function installStyles(panel){
  ...
  // Only apply daytime-theme if user explicitly chose light. Never auto-detect on first paint.
  panel.classList.toggle('daytime-theme', forceLight);
  ...
  if (!savedScheme && panel._hass?.states?.['sun.sun']) {
    ...
    panel.classList.toggle('daytime-theme', isDay);
  }
