...
import type { ArgusBreakpoint, ArgusWidgetDefinition, ArgusWidgetSize, DashboardLayoutStorage, Layout, Layouts } from './types';
...
function Host({ widget, node, editing, size, onSize, onHide, onReset }: HostProps) {
  ...
  useLayoutEffect(() => {
    ...
    if (editing) node.setAttribute('inert', '');
  ...
  return (
    <article className="argus-widget">
      <header className="argus-widget__edit-header">
        <button
          type="button"
          className="argus-widget__drag-handle"
          aria-label={`${getT('drag', 'Mover')} ${widget.title}`}
          title={getT('drag', 'Arrastrar para mover')}
        >
          ⋮⋮
        </button>
        <strong>{widget.title}</strong>
        <details className="argus-widget__options">
          <summary aria-label={`${getT('settings', 'Opciones')} ${widget.title}`} title={getT('settings', 'Opciones')}>
            •••
          </summary>
          <div className="argus-widget__menu">
            <span>{getT('size', 'Tamaño')}</span>
            <div>
              {(['S', 'M', 'L', 'XL'] as ArgusWidgetSize[]).map(value => (
                <button
                  type="button"
            ...
            <button type="button" onClick={onReset}>{getT('reset_widget', 'Restablecer widget')}</button>
            <button type="button" onClick={onHide}>{getT('hide_widget', 'Ocultar widget')}</button>
          </div>
        </details>
      </header>
...
export function ArgusDashboard({
  ...
  useEffect(() => {
    ...
    window.addEventListener('argus-lang-changed', onLang);
    return () => window.removeEventListener('argus-lang-changed', onLang);
  }, []);

  useEffect(() => {
    registerEditor(setEditing);
  }, [registerEditor]);
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if (event.key==='Escape') {
        setEditing(false);
        setMessage(getT('edit_dashboard_done', 'Edición finalizada'));
  ...
  const replaceItem = (id: string, nextItem: Layout, label: string) => {
    ...
    const current = (base[bp] as Layout[] | undefined) || [];
    const others = current.filter((x: Layout) => x.i !== id);
    const safe = hasCollision(others, nextItem) ? firstFreePosition(others, nextItem, COLS[bp]) : nextItem;
    const next = { ...base, [bp]: current.map((x: Layout) => (x.i === id ? safe : x)) };
    save(next, true);
    setMessage(label);
  };

  const chooseSize = (id: string, size: ArgusWidgetSize) => {
    const current = ((lastValid.current[bp] as Layout[] | undefined) || []).find((x: Layout) => x.i === id);
    if (!current) return;
    const d = clampSizeForBreakpoint(size, COLS[bp]);
    replaceItem(id, { ...current, ...d, x: Math.max(0, Math.min(current.x, COLS[bp] - d.w)) }, `${getT('size', 'Tamaño')} ${size}`);
  };

  const resetWidget = (id: string) => {
    const base = ((defaultLayouts[bp] as Layout[] | undefined) || []).find((x: Layout) => x.i === id);
    if (base) replaceItem(id, { ...base }, getT('reset_widget', 'Widget restablecido'));
  };

  const resize = (_layout: any, _old: any, value: any) => {
    if (!value?.i) return;
  ...
  const reset = async () => {
    ...
    setLayouts(clean);
    lastValid.current = clean;
    setGridKey(k => k + 1);
    save(clean, true);
    setMessage(getT('reset_dashboard_done', 'Diseño predeterminado restaurado'));
  };

  const currentLayout = (layouts[bp] as Layout[] | undefined) || [];
  if (!hydrated) {
    return (
      <section className="argus-dashboard" ref={containerRef}>
        <div className="argus-dashboard__feedback" aria-live="polite">
          {getT('loading_dashboard', 'Cargando tablero…')}
        </div>
      </section>
    );
  }
  ...
  return (
    <section className={`argus-dashboard ${editing ? 'argus-dashboard--editing' : ''}`} ref={containerRef}>
      <nav className="argus-dashboard__toolbar" aria-label={getT('edit_dashboard', 'Edición del tablero')}>
        <button type="button" onClick={() => setEditing(v => !v)}>
          {editing ? ('✓ ' + getT('edit_dashboard_done', 'Listo')) : ('❖ ' + getT('edit_dashboard', 'Editar tablero'))}
        </button>
        {editing && (
          <>
            <button type="button" className="argus-btn-reset-dashboard" onClick={reset}>
              {getT('reset_dashboard', 'Restablecer diseño')}
            </button>
            <div className="argus-dashboard__visibility" aria-label={getT('hide_widget', 'Widgets ocultos')}>
              {widgets.filter(w => visibility[w.id] === false).map(w => (
                <button type="button" key={w.id} onClick={() => setVisible(w.id, true)}>
                  {getT('show', 'Mostrar')} {w.title} {/* Mostrar {w.title} */}
                </button>
              ))}
            </div>
          </>
      ...
      <ErrorBoundary>
        <ResponsiveGridLayout
          ...
          resizeHandles={['se']}
          onBreakpointChange={(value: any) => setBp(value as ArgusBreakpoint)}
          onLayoutChange={(_: any, all: any) => {
            if (editing) {
              setLayouts(all);
          ...
          onResizeStop={resize}
          onDragStop={(_: any, __: any, value: any) => {
            if (!value?.i) return;
            const others = ((lastValid.current[bp] as Layout[] | undefined) || []).filter((x: Layout) => x.i !== value.i);
            if (hasCollision(others, value as Layout)) {
              setLayouts({ ...lastValid.current });
              setMessage(getT('position_collision', 'Posición bloqueada por colisión'));
              return;
            }
            replaceItem(value.i, value as Layout, getT('position_saved', 'Posición guardada'));
          }}
