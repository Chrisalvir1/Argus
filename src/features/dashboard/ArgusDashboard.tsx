...
function Host({widget,node,editing,size,onSize,onHide,onReset}:HostProps){const ref=useRef<HTMLDivElement>(null);useLayoutEffect(()=>{ref.current?.appendChild(node);node.draggable=false;node.querySelector(':scope > .panel-edit-overlay')?.remove();if(editing)node.setAttribute('inert','');else node.removeAttribute('inert')},[node,editing]);return <article className="argus-widget"><header className="argus-widget__edit-header"><button type="button" className="argus-widget__drag-handle" aria-label={`Mover ${widget.title}`} title="Arrastrar para mover">⋮⋮</button><strong>{widget.title}</strong><details className="argus-widget__options"><summary aria-label={`Opciones de ${widget.title}`} title="Opciones">•••</summary><div className="argus-widget__menu"><span>Tamaño</span><div>{(['S','M','L','XL']as ArgusWidgetSize[]).map(value=><button type="button" className={value===size?'active':''} onClick={()=>onSize(value)} key={value}>{value}</button>)}</div><button type="button" onClick={onReset}>Restablecer widget</button><button type="button" onClick={onHide}>Ocultar widget</button></div></details></header><div className={`argus-widget__content${widget.kind==='access-control'?' argus-widget__content--access':''}`} ref={ref}/></article>}
