...
class TestOptionsAndVisuals(unittest.TestCase):
 ...
 def test_buttons_use_glass_and_only_active_state_colors(self):
  s=(W/'argus-v2046-active-visuals.js').read_text();self.assertIn("state==='disarmed'",s);self.assertIn("rgba(255,255,255,.105)",s);self.assertIn('argus-action-active',s)
 def test_arming_shield_and_label_are_restored(self):
  s=(W/'argus-v2046-active-visuals.js').read_text();self.assertIn('argusArmingShield',s);self.assertIn('ESPERANDO SENSORES',s);self.assertIn('ARMANDO…',s)
