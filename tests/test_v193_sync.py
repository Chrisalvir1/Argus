...
class TestV193FrontendContracts(unittest.TestCase):
    ...
    def test_version_updated_to_195(self):
        """argus-panel.js header must reflect v2.0.41."""
        self.assertIn("v2.0.41", self.content, "argus-panel.js must mention v2.0.41")
...
class TestV193ManifestVersion(unittest.TestCase):
    def test_manifest_version_195(self):
        ...
        self.assertEqual(manifest["version"], "2.0.41", "manifest.json version must be 2.0.41")
...
class TestV193BootstrapCacheBust(unittest.TestCase):
    def test_bootstrap_version_195(self):
        ...
        with open(bootstrap_path) as f:
            content = f.read()
        self.assertIn("2.0.41", content, "argus-bootstrap.js must reference version 2.0.41 for cache-busting")
