...
class TestTriggerVoiceRuntime(unittest.TestCase):
 def test_only_announces_after_real_trigger(self):
  s=(C/'trigger_voice.py').read_text();self.assertIn('await original_trigger(self)',s);self.assertIn('self._alarm_state==AlarmControlPanelState.TRIGGERED',s);self.assertIn('not getattr(self,"_panic_active",False)',s)
