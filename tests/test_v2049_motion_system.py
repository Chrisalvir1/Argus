...
class TestV2049MotionSystem(unittest.TestCase):
    ...
    def test_profile_transition_survives_load_without_reanimating_every_load(self):
        self.assertIn("ensureTransition(state)", MOTION)
        self.assertIn("installStyles(this)", MOTION)
        self.assertIn("argus-motion-dashboard-prep", MOTION)
        self.assertNotIn("else frame(state,()=>animateDashboard(state))", MOTION)
    ...
    def test_responsive_rules_target_real_shadow_dom(self):
        self.assertIn("#w-access", RESPONSIVE)
        self.assertIn("#w-backup", RESPONSIVE)
        ...
        self.assertIn("justify-content:center", RESPONSIVE)
