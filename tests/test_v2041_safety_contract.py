...
class TestV2041SafetyContract(unittest.TestCase):
 def test_homekit_off_cancels_any_pending_state(self):
  s=(COMP/'safety_runtime.py').read_text();self.assertIn('pending or self._alarm_state == AlarmControlPanelState.ARMING',s);self.assertIn('cancelled_from_off',s);self.assertIn('async_announce_arming_cancelled',s)
