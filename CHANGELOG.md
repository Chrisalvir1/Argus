...
### Added
- **Opaque Frosted Profile Popover:** Deep non-transparent frosted background (`#0f172a` with `backdrop-filter: blur(48px) saturate(200%)`) eliminating background text bleed-through.
- **Direct PIN Configuration Modals in Profile:** Clicking "Cambiar" on Pin perfil Argus or Pin de Armado in the profile popover now directly opens the full interactive configuration modals.
- **High-Contrast Pure White Badge Text:** All user status and account badges (`CUENTA HA`, `PIN PERFIL ARGUS`, `INDEFINIDO`, etc.) now feature bright, pure white text (`#ffffff`) for maximum clarity and contrast on vivid colored backgrounds.
- **Wide Stacked SOS Mode Layout:** Full-size local emergency phone card with wide, spacious SOS devices selector grid positioned directly underneath.

## [2.2.81] - 2026-08-21

### Added
- **Direct PDF Activity Log Export:** The export history button now directly generates and downloads a formatted, multi-page `.pdf` document (`argus_historial_YYYY-MM-DD.pdf`) with complete event metadata, institutional header, and detailed audit records.
- **Unified Informative Status Card:** Combined SOS signals, notification channels, and audit log indicators into a single compact informative diagnostic card in Settings.
- **Full-Size Emergency Number & Wide SOS Selector:** The emergency phone card now matches the generous dimensions of the mode card, with a wide, spacious SOS device selector grid positioned directly underneath.
- **PIN Nomenclature Modernization:** Renamed "PIN Maestro" to **"Pin de Armado"** (*Arming PIN*) and "PIN de Acceso" to **"Pin perfil Argus"** (*Argus Profile PIN*) across all labels, settings, onboarding/first-run flows, modals, and localization dictionaries in 7 languages.

## [2.2.80] - 2026-08-21

### Added
- **Profile Popover Frosted Blur & Jelly Spring Physics:** High-fidelity frosted glass background (`backdrop-filter: blur(32px) saturate(190%)` and `background: rgba(18, 26, 40, 0.95)`) eliminating background text bleed-through, plus smooth spring jelly opening and closing physics (`@keyframes jellySpringIn` / `@keyframes jellySpringOut`).
- **Real-Time Sensor Synchronization:** Instant live updates of console sensor cards whenever sensors are added or removed from mode configurations.
